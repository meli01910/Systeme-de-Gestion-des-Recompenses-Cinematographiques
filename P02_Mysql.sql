...
SELECT * FROM P02_individu WHERE individu_nom REGEXP '(el|en)$';
/*--------------------------------------------------------------------------------------------------------------------------------------*/
...
WHERE recompense_nom IN ('meilleur acteur', 'meilleur actrice');
/*--------------------------------------------------------------------------------------------------------------------------------------*/
/*Par jointure externe.*/

/*--------------------------------------------------------------------------------------------------------------------------------------*/
SELECT individu_prenom, individu_nom, recompense_nom,statut
FROM P02_individu
left OUTER JOIN P02_nomination ON P02_individu.individu_id = P02_nomination.individu_id
left OUTER JOIN P02_recompense ON P02_nomination.recompense_id = P02_recompense.recompense_id
WHERE recompense_nom IN ('meilleur acteur', 'meilleur actrice')
UNION
SELECT individu_prenom, individu_nom, recompense_nom,statut
FROM P02_individu
right OUTER JOIN P02_nomination ON P02_individu.individu_id = P02_nomination.individu_id
right OUTER JOIN P02_recompense ON P02_nomination.recompense_id = P02_recompense.recompense_id
WHERE recompense_nom IN ('meilleur acteur', 'meilleur actrice');
...
La version avec UNION (simulant une jointure externe) renverra tous les individus de la table P02_individu, même ceux qui n'ont pas remporté de récompense ni été nominés, avec des valeurs NULL pour les colonnes de P02_nomination et P02_recompense.
Alors non les résultats ne seront pas identiques.*/

/*--------------------------------------------------------------------------------------------------------------------------------------*/
/*Par produit cartésien et restriction. (Ancienne Syntaxe)*/
/*La restriction dans la clause WHERE garantit que seules les lignes correspondant à la condition de jointure sont incluses.*/
/*--------------------------------------------------------------------------------------------------------------------------------------*/
select  individu_prenom,individu_nom,recompense_nom,statut
...
/*requete2: Affiche toutes les récompenses décernées lors de cérémonies annuelles */
...
SELECT ceremonie_nom, recompense_nom, ceremonie_pays
FROM P02_recompense
right OUTER JOIN P02_ceremonie P02c ON P02_recompense.ceremonie_id = P02c.ceremonie_id
WHERE ceremonie_type = 'Annuelle'
UNION
SELECT ceremonie_nom, recompense_nom, ceremonie_pays
FROM P02_recompense
...
La version avec UNION (simulant une jointure externe) renverra toutes les récompenses de la table P02_recompense,
...
/*requete3: Affiche les meilleurs films, le nom de la récompense, la cérémonie et l'année de remise*/
...
SELECT film_nom, annee_remise, recompense_nom, ceremonie_nom
FROM P02_film
left OUTER JOIN P02_nomination P02n ON P02_film.film_id = P02n.film_id
left OUTER JOIN P02_recompense P02r ON P02n.recompense_id = P02r.recompense_id
left OUTER JOIN P02_ceremonie P02c ON P02r.ceremonie_id = P02c.ceremonie_id
WHERE statut = 'gagnant' AND recompense_nom LIKE '%meilleur film%'
UNION
SELECT film_nom, annee_remise, recompense_nom, ceremonie_nom
FROM P02_film
right OUTER JOIN P02_nomination P02n ON P02_film.film_id = P02n.film_id
right OUTER JOIN P02_recompense P02r ON P02n.recompense_id = P02r.recompense_id
right OUTER JOIN P02_ceremonie P02c ON P02r.ceremonie_id = P02c.ceremonie_id
WHERE statut = 'gagnant' AND recompense_nom LIKE '%meilleur film%';
...
La version avec UNION (simulant une jointure externe) renverra tous les films de la table P02_film,
...
SELECT individu_nom, individu_prenom, statut,recompense_nom,film_nom
FROM P02_individu
left OUTER JOIN  P02_nomination P02n ON P02_individu.individu_id = P02n.individu_id
left OUTER JOIN P02_recompense P02r on P02r.recompense_id = P02n.recompense_id
left outer join P02_film P02f on P02n.film_id = P02f.film_id
where film_nom='Une place au soleil'
UNION
SELECT individu_nom, individu_prenom, statut,recompense_nom,film_nom
FROM P02_individu
right OUTER JOIN  P02_nomination P02n ON P02_individu.individu_id = P02n.individu_id
right OUTER JOIN P02_recompense P02r on P02r.recompense_id = P02n.recompense_id
right outer join P02_film P02f on P02n.film_id = P02f.film_id
where film_nom='Une place au soleil';
...
tandis que la jointure externe(l'approche avec UNION) retournera tous les lauréat avec des valeurs NULL pour les colonnes des tables non correspondantes.*/
...
/*Afficher les individus qui ont remporté à la fois un Oscar et un Golden Globe. Dans le cas d'un réalisateur, cela inclut la possibilité que le film lui-même ait remporté la catégorie "Meilleur film"*/
...
/*Affiche la liste des récompenses annuelles attribuées lors des cérémonies en France, excluant celles attribuées lors des festivals  en utilisant l'operateur "EXCEPT"*/
...
/*Affiche une liste tous les realisiteurs qui sont nominés*/
...
/*d-Une sous requete imbriquée*/
/*Affiche les informations des individus nominés pour une récompence dont le nom commence par meilleur actrice (comme meilleur actrice second role)*/
...
/* Affiche les films qui ont gagné des récompenses la meme année de leur création */
...
/*Comparaison : Les deux requêtes sont équivalentes en termes de résultats
...
select AVG(YEAR(current_date)-YEAR(individu_naissance)) as moyenne_age
from P02_individu;

/*--------------------------------------------------------------------------------------------------------------------------------------*/
/* question 7-Deux Requêtes avec les Fonctions d'Agrégation et la Clause GROUP BY*/

/*Requete 1 : Affiche le nombre de nominations par films*/
...
/*Requete 2 : Affiche la première année et la dérnière année d'organisation de chaque céremonie */

SELECT P02_ceremonie.ceremonie_nom, MIN(P02_nomination.annee_remise) AS Première_organisation, MAX(P02_nomination.annee_remise) AS Dérnière_organisation
FROM P02_nomination
...
SELECT P02_ceremonie.ceremonie_nom,ceremonie_description, COUNT(P02_recompense.recompense_id) AS NombreDeRecompense
FROM P02_ceremonie
LEFT JOIN P02_recompense ON P02_ceremonie.ceremonie_id = P02_recompense.ceremonie_id
GROUP BY P02_ceremonie.ceremonie_nom, ceremonie_description
...
FROM P02_recompense
JOIN P02_nomination ON P02_recompense.recompense_id = P02_nomination.recompense_id
