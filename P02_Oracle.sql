...
--affiche tous les individue dont le nom se termine par 'el' ou par 'en'
...
SELECT * FROM P02_individu WHERE  REGEXP_LIKE(individu_nom,'(el|en)$');
...
WHERE ceremonie_types = 'Annuelle';
------------------------------------
-- Syntaxe 2 de jointure interne
...
where P02_recompense.ceremonie_id = P02_ceremonie.ceremonie_id and ceremonie_types='Annuelle';
...
---------------------------------------------------------------
--requete intersect
--Affichez les individus qui ont remporté à la fois un Oscar et un Golden Globe. Dans le cas d'un réalisateur, cela inclut la possibilité que le film lui-même ait remporté la catégorie "Meilleur film".
...
SELECT recompense_nom, ceremonie_nom
FROM P02_recompense NATURAL JOIN P02_ceremonie
WHERE ceremonie_types = 'Festival';
-----------------------------------------------------------------------------------------------------
--question 4
...
--Affichez les individus ayant été nominés pour des récompenses individuelles
...
--affiche tous les gagnant en 2017
...
FROM P02_film
JOIN P02_nomination ON P02_film.film_id = P02_nomination.film_id
WHERE film_annee = ANY (
    SELECT DISTINCT annee_remise
    FROM P02_nomination
    WHERE P02_nomination.film_id = P02_film.film_id
)AND recompense_id IN (
    SELECT recompense_id
    FROM P02_recompense
...
------------------------------------------------------------------------------------------------------------------------------------
--question 6-Donner deux requêtes différentes utilisant les fonctions d'agrégation SQL
...
--Requete2 : Affiche la première année et la dérnière année d'organisation de chaque céremonie
